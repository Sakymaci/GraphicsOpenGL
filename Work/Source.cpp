...
glm::vec3	cameraPos = glm::vec3(9.0f, 0.0f, 0.0f),
...
void computeCameraMatrix() {
	...
	view	= glm::lookAt(cameraPos, cameraTarget+ cameraUpVector, cameraUpVector);
...
void display() {
	...
	if ((keyboard[GLFW_KEY_A]) || (keyboard[GLFW_KEY_LEFT]))
		cameraPos -= cameraSpeed * glm::normalize(glm::cross((cameraTarget - cameraPos), cameraUpVector)); // compute side moving vector

	if ((keyboard[GLFW_KEY_D]) || (keyboard[GLFW_KEY_RIGHT]))
		cameraPos += cameraSpeed * glm::normalize(glm::cross((cameraTarget - cameraPos), cameraUpVector)); // compute side moving vector

	// light follow the user viewing the scene, behaves as a headlamp 
	lightPos -= 10*cameraSpeed * glm::normalize(glm::cross((cameraTarget - lightPos), cameraUpVector));
	computeModelMatrix();
	computeCameraMatrix();
